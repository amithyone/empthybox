<?php $__env->startSection('title', 'SMS Verification Service - BiggestLogs'); ?>
...
<div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 md:py-8 pb-20 md:pb-8">
    ...
    <!-- Request SMS Number -->
    <div id="sms-data" data-services='<?php echo json_encode($services["services"] ?? [], 15, 512) ?>' data-countries='<?php echo json_encode($countries["countries"] ?? [], 15, 512) ?>' class="hidden"></div>
    ...
    <div class="bg-dark-200 border-2 border-dark-300 rounded-xl shadow-lg p-4 md:p-6 mb-6">
        ...
        <?php if(isset($services['success']) && !$services['success']): ?>
        <div class="bg-red-500/20 border-2 border-red-500/50 rounded-xl p-4 mb-4">
            ...
            <p class="text-sm text-red-200 mt-1"><?php echo e($services['error'] ?? 'Unknown error occurred'); ?></p>
            <p class="text-xs text-red-300 mt-2">Provider: <?php echo e($services['provider'] ?? 'Unknown'); ?></p>
        </div>
        <?php endif; ?>
        
        <?php if(isset($countries['success']) && !$countries['success']): ?>
        <div class="bg-yellow-500/20 border-2 border-yellow-500/50 rounded-xl p-4 mb-4">
            ...
            <p class="text-sm text-yellow-200 mt-1"><?php echo e($countries['error'] ?? 'Unknown error occurred'); ?></p>
        </div>
        <?php endif; ?>
        
        <?php if(isset($services['success']) && $services['success'] && empty($services['services'])): ?>
        <div class="bg-yellow-500/20 border-2 border-yellow-500/50 rounded-xl p-4 mb-4">
...
async function loadAvailableCountriesForService(serviceId) {
    try {
        ...
        const response = await fetch('<?php echo e(route("sms.service-countries")); ?>', {
            method: 'POST',
            body: formData,
            headers: {
...
async function refreshCountryPricing(countryId) {
    try {
        ...
        const response = await fetch('<?php echo e(route("sms.pricing")); ?>', {
            method: 'POST',
            body: formData,
...
document.getElementById('sms-request-form').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo e(route("sms.request-number")); ?>', {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '<?php echo e(route("sms.inbox")); ?>';
            }, 2000);
        } else {
...
function pollForMessages(orderId) {
    const interval = setInterval(async () => {
        try {
            ...
            const response = await fetch('<?php echo e(route("sms.check-status")); ?>', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\LENOVO LEGION\Documents\Biggestlogs\resources\views/sms/index.blade.php ENDPATH**/ ?>