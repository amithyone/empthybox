...
<?php $__env->startSection('title', 'Wallet - BiggestLogs'); ?>
...
<div class="max-w-4xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 md:py-8 pb-20 md:pb-8">
    ...
    <div class="bg-gradient-to-r from-red-accent via-yellow-accent to-red-accent rounded-xl shadow-2xl shadow-red-accent/30 p-6 md:p-8 text-white mb-6 md:mb-8">
        ...
        <p class="text-4xl md:text-5xl font-bold mb-4">₦<?php echo e(number_format($wallet->balance ?? 0, 2)); ?></p>
        <div class="grid grid-cols-2 gap-4 text-xs md:text-sm">
            <div>
                <p class="opacity-80">Total Deposited</p>
                <p class="text-lg md:text-xl font-semibold">₦<?php echo e(number_format($wallet->total_deposited ?? 0, 2)); ?></p>
            </div>
            <div>
                <p class="opacity-80">Total Withdrawn</p>
                <p class="text-lg md:text-xl font-semibold">₦<?php echo e(number_format($wallet->total_withdrawn ?? 0, 2)); ?></p>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-4">
            ...
            <div class="border-b border-dark-300 pb-4 last:border-0">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <p class="font-semibold text-gray-200"><?php echo e($transaction->description); ?></p>
                        <p class="text-xs md:text-sm text-gray-400"><?php echo e($transaction->created_at->format('M d, Y h:i A')); ?></p>
                        <?php if($transaction->gateway): ?>
                        <p class="text-xs text-gray-500">Via <?php echo e(ucfirst($transaction->gateway)); ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="text-left sm:text-right">
                        <p class="font-bold text-lg <?php echo e($transaction->type === 'deposit' || $transaction->type === 'refund' ? 'text-green-400' : 'text-red-accent'); ?>">
                            <?php echo e($transaction->type === 'deposit' || $transaction->type === 'refund' ? '+' : '-'); ?>₦<?php echo e(number_format($transaction->amount, 2)); ?>

                        </p>
                        <span class="text-xs px-2 py-1 rounded 
                            <?php echo e($transaction->status === 'completed' ? 'bg-green-600/20 text-green-400 border border-green-500/30' : 'bg-yellow-accent/20 text-yellow-accent border border-yellow-accent/30'); ?>">
...
document.getElementById('deposit-form')?.addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo e(route("wallet.deposit")); ?>', {
            method: 'POST',
            body: formData,
            headers: {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/biggestlogs/resources/views/wallet/index.blade.php ENDPATH**/ ?>