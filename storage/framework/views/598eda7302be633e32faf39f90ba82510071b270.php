...
<?php $__env->startSection('title', 'Dashboard - BiggestLogs'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 md:py-8 pb-20 md:pb-8">
    <h1 class="text-2xl md:text-3xl font-bold mb-4 md:mb-6 gradient-text">Welcome back, <?php echo e($user->name); ?>! 👋</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
        <div class="bg-dark-200 border-2 border-dark-300 p-4 md:p-6 rounded-xl shadow-lg hover:border-yellow-accent/50 transition">
            <div class="text-xl md:text-2xl font-bold bg-gradient-to-r from-red-accent to-yellow-accent bg-clip-text text-transparent"><?php echo e($stats['total_orders']); ?></div>
            <div class="text-xs md:text-sm text-gray-400 mt-1">Total Orders</div>
        </div>
        <div class="bg-dark-200 border-2 border-dark-300 p-4 md:p-6 rounded-xl shadow-lg hover:border-yellow-accent/50 transition">
            <div class="text-xl md:text-2xl font-bold text-green-400">₦<?php echo e(number_format($stats['wallet_balance'], 2)); ?></div>
            <div class="text-xs md:text-sm text-gray-400 mt-1">Wallet Balance</div>
        </div>
        <div class="bg-dark-200 border-2 border-dark-300 p-4 md:p-6 rounded-xl shadow-lg hover:border-yellow-accent/50 transition">
            <div class="text-xl md:text-2xl font-bold text-yellow-accent"><?php echo e($stats['pending_orders']); ?></div>
            <div class="text-xs md:text-sm text-gray-400 mt-1">Pending Orders</div>
        </div>
        <div class="bg-dark-200 border-2 border-dark-300 p-4 md:p-6 rounded-xl shadow-lg hover:border-yellow-accent/50 transition">
            <div class="text-xl md:text-2xl font-bold text-red-accent"><?php echo e($stats['open_tickets']); ?></div>
    ...
    <div class="bg-gradient-to-r from-yellow-accent/20 via-red-accent/10 to-yellow-accent/20 border-2 border-yellow-accent/30 rounded-xl shadow-lg p-4 md:p-6 mb-6 md:mb-8">
        <div class="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-4">
            <div class="flex-1">
                ...
                <?php if(\App\Models\Setting::get('sms_coming_soon', false) && !auth()->user()->is_admin): ?>
            ...
            <?php else: ?>
            <a href="<?php echo e(route('sms.select')); ?>" class="bg-gradient-to-r from-red-accent to-yellow-accent hover:from-red-dark hover:to-yellow-dark text-white px-6 py-3 rounded-xl font-semibold transition glow-button relative shadow-lg shadow-red-accent/30 whitespace-nowrap">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4 mb-6 md:mb-8">
        <a href="<?php echo e(route('products.index')); ?>" class="bg-gradient-to-br from-red-accent to-yellow-accent text-white p-4 md:p-6 rounded-xl shadow-lg text-center hover:shadow-xl hover:shadow-yellow-accent/50 transition transform hover:scale-105">
            <div class="text-2xl md:text-3xl mb-2">🛍️</div>
            <div class="font-semibold text-sm md:text-base">Shop</div>
        </a>
        <a href="<?php echo e(route('orders.index')); ?>" class="bg-dark-200 border-2 border-dark-300 text-gray-300 p-4 md:p-6 rounded-xl shadow-lg text-center hover:border-yellow-accent/50 transition transform hover:scale-105">
            <div class="text-2xl md:text-3xl mb-2">📦</div>
            <div class="font-semibold text-sm md:text-base">My Orders</div>
        </a>
        <a href="<?php echo e(route('wallet.index')); ?>" class="bg-dark-200 border-2 border-dark-300 text-gray-300 p-4 md:p-6 rounded-xl shadow-lg text-center hover:border-yellow-accent/50 transition transform hover:scale-105">
            <div class="text-2xl md:text-3xl mb-2">💰</div>
            <div class="font-semibold text-sm md:text-base">Wallet</div>
        </a>
        <?php if(!\App\Models\Setting::get('sms_coming_soon', false) || auth()->user()->is_admin): ?>
        <a href="<?php echo e(route('sms.inbox')); ?>" class="bg-dark-200 border-2 border-dark-300 text-gray-300 p-4 md:p-6 rounded-xl shadow-lg text-center hover:border-yellow-accent/50 transition transform hover:scale-105">
            <div class="text-2xl md:text-3xl mb-2">📬</div>
        ...
        <?php endif; ?>
        <a href="<?php echo e(route('tickets.index')); ?>" class="bg-dark-200 border-2 border-dark-300 text-gray-300 p-4 md:p-6 rounded-xl shadow-lg text-center hover:border-yellow-accent/50 transition transform hover:scale-105">
            <div class="text-2xl md:text-3xl mb-2">🎫</div>
    ...
    <div class="bg-dark-200 border-2 border-dark-300 rounded-xl shadow-2xl shadow-red-accent/10 p-4 md:p-6 mb-6 md:mb-8">
        ...
        <div class="space-y-4">
            ...
            <div class="border-b border-dark-300 pb-4 last:border-0">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <div class="font-semibold text-gray-200"><?php echo e($order->product->name); ?></div>
                        <div class="text-xs md:text-sm text-gray-400">Order #<?php echo e($order->order_number); ?></div>
                        <div class="text-xs md:text-sm mt-1">
                            <span class="px-2 py-1 rounded <?php echo e($order->status === 'completed' ? 'bg-green-600/20 text-green-400' : 'bg-yellow-accent/20 text-yellow-accent'); ?>">
                                <?php echo e(ucfirst($order->status ?? 'pending')); ?>

                            </span>
                        </div>
                    </div>
                    <a href="<?php echo e(route('orders.show', $order)); ?>" class="bg-gradient-to-r from-red-accent to-yellow-accent hover:from-red-dark hover:to-yellow-dark text-white px-4 py-2 rounded-lg font-medium transition text-sm md:text-base shadow-lg shadow-red-accent/30 self-start sm:self-auto">
                        View
                    </a>
                </div>
            </div>
        ...
        <div class="mt-4 text-center">
            <a href="<?php echo e(route('orders.index')); ?>" class="text-yellow-accent font-semibold hover:text-red-accent transition">View All Orders →</a>
        </div>
        <?php else: ?>
        <div class="text-center py-8">
            <p class="text-gray-400 mb-4">No orders yet. Start shopping!</p>
            <a href="<?php echo e(route('products.index')); ?>" class="inline-block bg-gradient-to-r from-red-accent to-yellow-accent hover:from-red-dark hover:to-yellow-dark text-white px-6 py-3 rounded-lg font-medium transition shadow-lg shadow-red-accent/30">
                Browse Products
            </a>
        </div>
        <?php endif; ?>
...
document.getElementById('pin-setup-form')?.addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo e(route("setup-pin")); ?>', {
            method: 'POST',
            body: formData,
            headers: {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/biggestlogs/resources/views/dashboard/index.blade.php ENDPATH**/ ?>